    ...
    <div className="fixed bottom-5 left-5 z-40 flex max-w-full flex-col gap-3">
      <div className="rounded-[var(--radius-lg)] border border-border-soft bg-background-elevated p-4 shadow-[var(--shadow-soft)]">
        ...
        <div className="mt-3 flex items-center gap-3">
          {items.map((item) => (
            <div key={item.id} className="relative flex items-center gap-2 rounded-[var(--radius-md)] border border-border-soft bg-background px-2 py-2 text-xs">
              <div className="relative h-10 w-10 overflow-hidden rounded-[var(--radius-sm)] bg-background-muted">
                <Image
                  src={item.heroImage ?? "https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=400&q=80"}
                  alt={item.name}
                  fill
                  sizes="40px"
